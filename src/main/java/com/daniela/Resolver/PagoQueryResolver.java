...
public class PagoQueryResolver implements GraphQLQueryResolver {
    @Autowired
    private PagoRepository pagoRepository;

    @QueryMapping 
    public EntityPage<Pago> getPagos(@Argument int page, @Argument int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Pago> entityPage = pagoRepository.findAll(pageable);
    ...
    @QueryMapping 
    public Pago getPago(@Argument Long pagoId) {
        return pagoRepository.findById(pagoId).orElse(null); // Método que deberías definir en el repositorio
    }
}
